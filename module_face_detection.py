...
def multiple_file_camera_face_rec_and_cropping(LIST_OUTPUT_FILE_NAME, LIST_OF_INPUT_FILE_NAME, FOURCC1='M', FOURCC2='J', FOURCC3='P',FOURCC4 ='G', ADD_STR_CROPPED_FILE_NAME = '_cropped', INPUT_FILE_NAME_EXTENSION = '.mkv', OUPUT_FILE_NAME_EXTENSION = '.avi', CROPPED_WIDTH = 110, CROPPED_HEIGHT = 105, SHIFT_RIGHT = -50, SHIFT_DOWN = 0, OUTPUT_FPS = 24,  ENABLE_FACE_RECOGNITION_TRACKING_CROPING = True, WHOLE_FACE_PROFILE = False, LIPS_PROFILE = False, LOAD_FACE_LANDMARKS = True, POINT_LAND_MARK_TRACKING = False, LAND_MARK_TRACKING_NUMBER = 1, LAND_MARK_LIP_TRACKING = True, CAPTURE_FACE_LANDMARKS = True, DISPLAY_FACE_LANDMARKS = False, SAVE_LANDMARK_TRACKING_RESULTS = False, SAVE_LANDMARK_TRACKING_RESULTS_NAME = 'Record', SAVE_WHOLE = False, SAVE_CROPPED = True, DISPLAY_WHOLE = False, DISPLAY_CROPPED = False, FUTURE_KILL_SWITCH = False, ENABLE_CUBIC_LAND_MARK_TRACKING = True, CUBIC_LAND_MARK_POINT_TOP = 34,CUBIC_LAND_MARK_POINT_LEFT = 49, CUBIC_LAND_MARK_POINT_BOTTOM = 9, CUBIC_LAND_MARK_POINT_RIGHT = 55, FLIP = True, FLIP_ARGUMENT = 1, SHAPE_PREDICTOR_NUMBER_OF_LANDMARK_POINTS = 68):
    ...
    cubic_land_mark_point_bottom = CUBIC_LAND_MARK_POINT_BOTTOM
    cubic_land_mark_point_right = CUBIC_LAND_MARK_POINT_RIGHT
    
    flip = FLIP
    flip_argument = FLIP_ARGUMENT
    output_file_name_extension = OUPUT_FILE_NAME_EXTENSION
    enable_face_recognition_tracking_croping = ENABLE_FACE_RECOGNITION_TRACKING_CROPING
    whole_face_profile = WHOLE_FACE_PROFILE
    lips_profile = LIPS_PROFILE
    load_face_landmarks = LOAD_FACE_LANDMARKS
    point_land_mark_tracking = POINT_LAND_MARK_TRACKING
    land_mark_tracking_number = LAND_MARK_TRACKING_NUMBER
    land_mark_lip_tracking = LAND_MARK_LIP_TRACKING
    capture_face_landmarks = CAPTURE_FACE_LANDMARKS
    display_face_landmarks = DISPLAY_FACE_LANDMARKS
    
    add_str_cropped_file_name = ADD_STR_CROPPED_FILE_NAME
    
    save_whole = SAVE_WHOLE
    save_cropped = SAVE_CROPPED
    display_whole = DISPLAY_WHOLE
    display_cropped = DISPLAY_CROPPED
    future_kill_switch = FUTURE_KILL_SWITCH
    save_landmark_tracking_results = SAVE_LANDMARK_TRACKING_RESULTS
    save_record_file_name = SAVE_LANDMARK_TRACKING_RESULTS_NAME
    
    #PURE_VIDEO_CROPPED_SENTENCE_OR_WORD_CHUNK_SAMPLES_INFO = []
    #LIST_FACE_DETECTION_RESULT_PER_FRAME = []
    LIST_RATIO_OF_DETECTED_FACES_PER_FRAME = []
    LIST_OF_NUMBER_OF_FRAMES = []
    LIST_OF_LAND_MARK_RESULTS = []
    LIST_CROPPED_VIDEO_FILENAME = []
    LIST_OF_MEASURED_RECORDING_PROCESSING_TIME = []
    LIST_OF_MEASURED_FPS = []
    LIST_OF_INPUT_VIDEO_FPS = []
    for i in range(0, len(LIST_OF_INPUT_FILE_NAME)):        
        
        number_of_camera = LIST_OF_INPUT_FILE_NAME[i]
        output_file_name = LIST_OUTPUT_FILE_NAME[i]
        ...
        LIST_RATIO_OF_DETECTED_FACES_PER_FRAME.append(ratio_of_detected_faces_per_frame)
        LIST_OF_NUMBER_OF_FRAMES.append(number_of_frames)
        LIST_OF_LAND_MARK_RESULTS.append(landmark_tracking_result)
        LIST_OF_MEASURED_RECORDING_PROCESSING_TIME.append(measured_recording_processing_time)
        LIST_OF_MEASURED_FPS.append(measured_fps)
        LIST_OF_INPUT_VIDEO_FPS.append(input_vid_fps)
        #LAND_MARK_RESULTS.append()
        
    #     # dictionary made out of lists 
    dict = {'LIST_CROPPED_VIDEO_FILENAME': LIST_CROPPED_VIDEO_FILENAME, 'LIST_RATIO_OF_DETECTED_FACES_PER_FRAME': LIST_RATIO_OF_DETECTED_FACES_PER_FRAME, 'LIST_OF_NUMBER_OF_FRAMES': LIST_OF_NUMBER_OF_FRAMES, 'LIST_OF_LAND_MARK_RESULTS' : LIST_OF_LAND_MARK_RESULTS, 'LIST_OF_MEASURED_RECORDING_PROCESSING_TIME': LIST_OF_MEASURED_RECORDING_PROCESSING_TIME, 'LIST_CROPPED_VIDEO_FILENAME':LIST_CROPPED_VIDEO_FILENAME, 'LIST_OF_MEASURED_FPS':LIST_OF_MEASURED_FPS, 'LIST_OF_INPUT_VIDEO_FPS':LIST_OF_INPUT_VIDEO_FPS} 
